...
import fs from 'fs';
import { jsonDb, datetime, prompt, notify, html_game_list, handleSIGINT } from './src/util.js';
...
async function ensureLoggedIn() {
  ...
  console.log('Navigated to Playstation page');
...
async function main() {
  ...
  try {
    // Step 2: Login to PlayStation Network
    await ensureLoggedIn();

    // Step 3: Claim PS Plus games
    if (cfg.ps_plus_games) {
      await claimPSPlusGames();
    }

    // Step 4: Claim Game Catalog games
    if (cfg.ps_game_catalog) {
      console.log('GAME CATALOG - NOT IMPLEMENTED YET');
    }

    // Step 5: Claim Classics Catalog games
    if (cfg.ps_classics_catalog) {
      console.log('CLASSICS CATALOG - NOT IMPLEMENTED YET');
    }

    console.log('\nFinished claiming playstation games!');
    writeFileSync('data/playstation.json', JSON.stringify(db.data, null, 2));
    ...
    if (notify_games.filter(g => g.status !== 'existed').length) {
      notify(`playstation (${user}):<br>${html_game_list(notify_games)}`);
    }
...
async function doLoginIfNeeded() {
  ...
  if (loginLink) {
    console.log('Not signed in to playstation. Navigating to login page...');
...
async function doLogin() {
  await page.waitForSelector('#signin-entrance-input-signinId');
  if (!cfg.debug) context.setDefaultTimeout(cfg.login_timeout); // give user some extra time to log in
  console.info(`Login timeout is ${cfg.login_timeout / 1000} seconds!`);

  // ### FETCH EMAIL/PASS
  if (cfg.ps_email && cfg.ps_password) console.info('Using email and password from environment.');
  else console.info(
...
async function claimPSPlusGames() {
  ...
  const monthlyGamesBlock = await page.locator('.cmp-experiencefragment--wn-latest-monthly-games-content').first();

  const monthlyGamesLocator = await monthlyGamesBlock.locator('.btn--cta').all();

  const monthlyGamesPageLinks = await Promise.all(
    monthlyGamesLocator.map(async el => {
      const urlSlug = await el.getAttribute('href');
      // standardize URLs
...
async function dumpHtml(pg, errorContext) {
  try {
    ...
    const filename = `/tmp/playstation-debug-${timestamp}.html`;
